...
echo -e "Combined signature activities vs germline gene hits with dCRT - single binomial distribution (n=1)." | tee -a $LOGFILE
...
python3 ../src/signatures/associations/cancGeneHit/samples.py ${filename_samples} germline | tee -a $LOGFILE
...
python3 ../src/signatures/associations/cancGeneHit/targets_germline.py ${filename_samples} ${filename_targets} $n | tee -a $LOGFILE
