...
if __name__=='__main__':
    ...
    for index, row in cohorts.iterrows():
        ...
        stats_df["best_solution"] = stats_df.Signatures.map(lambda x: "*" in x).astype(int)
        stats_df["Signatures"]=np.array([str(sig).rstrip("*") for sig in list(stats_df.Signatures)]).astype(int)
        stats_df["Matrix Frobenius%"] = stats_df["Matrix Frobenius%"].map(lambda x: x.rstrip("%")).astype(float)
        stats_df.rename({"Signatures":"signatures",
                        "Matrix Frobenius%":"matrix_frobenius",
                        "Minimum Stability":"min_stability",
                        "Stability (Avg Silhouette)":"avg_stability"}, axis=1, inplace=True)
        # Add file paths for Signatures and Standard Errors
        stats_df["signatures_file"] = [f"{row.cohort_dir}/All_Solutions/{sig_type}_{sig}_Signatures/Signatures/{sig_type}_S{sig}_Signatures.txt" for sig in stats_df.signatures]
        stats_df["signatures_SE_file"] = [f"{row.cohort_dir}/All_Solutions/{sig_type}_{sig}_Signatures/Signatures/{sig_type}_S{sig}_Signatures_SEM_Error.txt" for sig in stats_df.signatures]
        stats_df["activities_file"] = [f"{row.cohort_dir}/All_Solutions/{sig_type}_{sig}_Signatures/Activities/{sig_type}_S{sig}_NMF_Activities.txt" for sig in stats_df.signatures]
        stats_df["samples_file"] = [f"{row.cohort_dir}/Samples.txt" for sig in stats_df.signatures]
        stats_df["cohort"] = row.cohort

        # Decomposed Signatures
        if sig_type=='SBSSV':
            stats_df["signatures_decomposed_file"] = [f"{row.cohort_dir}/All_Solutions/{sig_type}_{sig}_Signatures/Decompose_Solution/Signatures/Decompose_Solution_Signatures.txt" for sig in stats_df.signatures]
    ...
    full_stats_df.to_csv(f"{output_dir}/all_cohort_stats.tsv", index=False, header=True, sep="\t")
