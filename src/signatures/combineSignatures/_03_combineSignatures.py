...
import SigProfilerAssignment, SigProfilerExtractor
from SigProfilerAssignment import Analyzer as Analyze, decompose_subroutines
...
def cosineSimilarityReduce(signatures, threshold=0.8):
    ...
    # Generate dataframe of cosine similarities
    sigmat = np.array(signatures)/np.sqrt(np.sum(np.array(signatures)**2,
                                                                  axis=0))[None,:]
    ...
    cossim_df = pd.DataFrame({'cos_sim':np.triu(cosine_similarity_mtx).flatten(),
                              'sig_x':XX.flatten(),
                              'sig_y':YY.flatten()})
    # Drop same signatures and lower triangle
    cossim_df = cossim_df[(cossim_df['sig_x']!=cossim_df['sig_y'])&(cossim_df['cos_sim']>threshold)]
    # Drop signatures in same cohort
    cossim_df = cossim_df[cossim_df.sig_x.map(lambda x: x.split('-')[1])!=cossim_df.sig_y.map(lambda x: x.split('-')[1])]
    ...
    while len(cossim_df)>0:
        ...
        for row in cossim_df.iterrows():
            if (row[1].sig_x in mapping[i]) & (row[1].sig_y.split('-')[1] not in cohort_set):
                if (len(set(mapping[i])-set(cossim_df.sig_x[cossim_df.sig_y==row[1].sig_y]))==0):
                    mapping[i].append(row[1].sig_y)
                    cohort_set.append(row[1].sig_y.split('-')[1])
                drop_rows.append(row[0])
            elif (row[1].sig_y in mapping[i]) & (row[1].sig_x.split('-')[1] not in cohort_set):
                if (len(set(mapping[i])-set(cossim_df.sig_y[cossim_df.sig_x==row[1].sig_x]))==0):
...
def orderSignatures(signatures):
    ...
    mut_type  = np.array([re.search("T*([A-Za-z]+)([0-9]+)([a-zA-Z]*)", sig).group(1) for sig in signatures]) # type: ignore
    mut_index = np.array([re.search("([A-Za-z]+)([0-9]+)([a-zA-Z]*)", sig).group(2) for sig in signatures]).astype(int) # type: ignore
    mut_exten = np.array([re.search("([A-Za-z]+)([0-9]+)([a-zA-Z]*)", sig).group(3) for sig in signatures]) # type: ignore
...
def deconvolve_sigs(signatures, output, signature_database,
                    nnls_add_penalty=0.05, nnls_remove_penalty=0.01, initial_remove_penalty=0.05,
                    genome_build="GRCh37", cosmic_version=3.2, make_plots=True,
    ...
    try:
        ...
        print ("The {} folder could not be created".format("output"))
                                                                ...
                                                                # Decomposition
                                                                #################
    try: processAvg = pd.read_csv(signatures,sep='\t', index_col=0)
    except:
        try: processAvg=signatures
        except:
    ...
    if mutation_type == "78": mutation_context = "DBS78"
    elif mutation_type == "83": mutation_context = "ID83"
    elif mutation_type=="48": mutation_context = "CNV48"
    else: mutation_context = "SBS"+mutation_type

    #creating list of mutational type to sync with the vcf type input
    signature_names = decompose_subroutines.make_letter_ids(idlenth = processAvg.shape[1], mtype = mutation_context)
    ...
    if len(pd.read_csv(signature_database, nrows=1, index_col=0, sep="\t").keys())==0:
        ...
        final_signatures["dictionary"] = {sigid:[sigid] for sigid in final_signatures["newsigids"]}
        final_signatures["activity_percentages"] = [[100] for sigid in final_signatures["newsigids"]]
    ...
    final_signatures = decompose_subroutines.signature_decomposition(
                                                   processAvg, mutation_type, layer_directory2,
                                                   genome_build=genome_build,
                                                   signature_database=signature_database,
                                                   mutation_context=mutation_context,
                                                   add_penalty=0.05, connected_sigs=connected_sigs,
                                                   remove_penalty=0.01,
...
if __name__=='__main__':
    ...
    best_solutions_file, cosmic_file, output_dir, sig_type, sample_id_type = sys.argv[1:]
    ...
    for index, row in best_solution.iterrows():
        ...
        cohort_sigs.rename(columns={key:f"{key}-{row.cohort}" for key in cohort_sigs.columns}, inplace=True)
        ...
        cohort_se.rename(columns={key:f"{key}-{row.cohort}" for key in cohort_se.columns}, inplace=True)

        # Add cohort mutations and signatures to allSignatures table
        columns = [col for col in cohort_sigs.columns if not col in decomposed_sigs.columns]
        decomposed_sigs = pd.merge(decomposed_sigs, cohort_sigs[columns], how='right', left_index=True, right_index=True)
        decomposed_sigs_se = pd.merge(decomposed_sigs_se, cohort_se, how='right', left_index=True, right_index=True)
        mutations = pd.merge(mutations, cohort_mutations, how='right', left_index=True, right_index=True)
    ...
    decomposed_sigs_se.to_csv(f"{output_dir}/Decomposed_Solution_Signature_se.tsv", sep="\t")
    ...
    while iteration<50:
        ...
        if not os.path.exists(output_iter): os.makedirs(output_iter)
        ...
        for i in range(len(grouped_signatures)):
            if i<26: grouped_signatures[f"{string.ascii_uppercase[i]}"]=grouped_signatures.pop(i)
            else: grouped_signatures[f"{string.ascii_uppercase[i//26+1]}{string.ascii_uppercase[i%26]}"]=grouped_signatures.pop(i)
        ...
        for new_sig, old_sigs in grouped_signatures.items():
            if len(old_sigs)==1:
                if sig_type=="SBS288":
                    grouped_signature_values[new_sig] = decomposed_sigs[old_sigs[0]].groupby(decomposed_sigs.index.str[2:9]).sum()[grouped_signature_values.index]
                else:
                    grouped_signature_values[new_sig] = decomposed_sigs[old_sigs[0]][grouped_signature_values.index]
                grouped_signature_se[new_sig] = decomposed_sigs_se[old_sigs[0]]
            else:
                signature, signature_se = posteriorSignatures(decomposed_sigs[old_sigs], decomposed_sigs_se[old_sigs])
                if sig_type=="SBS288":
                    grouped_signature_values[new_sig] = signature.groupby(decomposed_sigs.index.str[2:9]).sum()[grouped_signature_values.index]
                else:
                    grouped_signature_values[new_sig] = signature[grouped_signature_values.index]
                grouped_signature_se[new_sig] = signature_se

            if cosmic.shape[1]>0:
                max_cossim = np.max(cosineSimilarity(grouped_signature_values[new_sig], cosmic))
            else:
                max_cossim = 0
            novel_candidates = pd.concat([novel_candidates,
                                        pd.DataFrame([[new_sig, old_sigs, len(old_sigs), max_cossim]],
                                                columns=['new_sig', 'old_sigs', 'cohort_count', 'max_cosmic_similarity'])])
        # Order groups by max cosine similarity with a reference signature -> take signature with lowest similarity
        novel_sig = novel_candidates.sort_values('max_cosmic_similarity').iloc[0].new_sig
        print(novel_sig)
        cosmic[f"{sig_type}{string.ascii_uppercase[iteration]}-Pan"] = grouped_signature_values[novel_sig]
        combined_signature_se[f"{sig_type}{string.ascii_uppercase[iteration]}-Pan"] = grouped_signature_se[novel_sig]
        grouped_signatures_cosmic[f"{sig_type}{string.ascii_uppercase[iteration]}-Pan"] = grouped_signatures[novel_sig]

        iteration+=1

    # Save combined signature uncertainties
    combined_signature_se.to_csv(f"{output_dir}/Combined_Solution_Signatures_SE.tsv", sep="\t")
    ...
    for index, row in best_solution.iterrows():
        ...
        # Run decompose fit
        Analyze.decompose_fit(row.samples_file,
                               output,
        ...
        cohort_acts = pd.read_csv(f"{output}/Decompose_Solution/Activities/Decompose_Solution_Activities.txt", sep="\t")\
    ...
    if sample_id_type=="tumour_sample_platekey":
        cancer_analysis = cancer_analysis = pd.read_csv(f"{DATA_DIR}/V11/V11_reheadered/cancer_analysis.tsv", sep="\t",
                                usecols=["participant_id", "tumour_sample_platekey", "germline_sample_platekey"])
        cancer_analysis["sample_id"] = cancer_analysis["participant_id"].astype(str)+"_"+\
                                    cancer_analysis["tumour_sample_platekey"]+"_"+\
                                    cancer_analysis["germline_sample_platekey"]
        combined_acts = pd.merge(combined_acts.reset_index(), cancer_analysis[['sample_id','tumour_sample_platekey']],
                                left_on="Samples", right_on="tumour_sample_platekey", how='left')\
                                        .drop(["Samples","tumour_sample_platekey"], axis=1).rename({"sample_id":"Samples"}, axis=1).set_index("Samples")
    elif sample_id_type=="pid_germline_tumour":
        combined_acts.set_index(combined_acts.index.map(lambda x: x.split('.')[0]+"_"+"_".join(x.split('.tumo')[1].split("_norm")) \
                                                            if "tumo" in x else \
                                                            "_".join(np.array(x.split("_"))[[0,3,4,1,2]]) ), inplace=True)
        print(combined_acts.head())

    # Save results
    signature_order = combined_acts.keys()[orderSignatures(combined_acts.keys())]
    combined_acts[signature_order].to_csv(f"{output_dir}/Combined_Solution_Activities.tsv", sep="\t", index=True)

    # Save signature group mappings
    pd.DataFrame(grouped_signatures_cosmic.items(), columns=['new_signatures', 'old_signatures'])\
