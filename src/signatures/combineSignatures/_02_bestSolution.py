...
if __name__ == "__main__":
    ...
    for cohort in cohorts:
        ...
        best_solution = estimate_solution(
            ...
            ],  # f"{cohort_dir}/All_solutions_stat.csv",
            genomes=cohort_stats[cohort_stats.cohort == cohort].samples_file.iloc[
                0
            ],  # f"{cohort_dir}/Samples.txt",
        ...
        solution_df = pd.DataFrame(
            cohort_stats[cohort_stats.cohort == cohort]
            .set_index("signatures")
            .loc[[best_solution]]
        )
        solution_df["cohort"] = cohort
        solution_df["samples_file"] = cohort_stats[
            cohort_stats.cohort == cohort
        ].samples_file.iloc[0]
