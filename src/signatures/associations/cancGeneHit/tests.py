...
if __name__=="__main__":
    ...
    samples_file, signatures_file, targets_file, tests_file, tests_file_binary, n = sys.argv[1:]
    min_sig_count=10
    min_tgt_count=5
    ...
    if True:
        mock_genes = targets.keys()[[bool(re.search("^MOCK[0-9A-Z]+$", tgt)) for tgt in targets.keys()]]
        print(mock_genes)
        targets = targets[np.intersect1d(targets.keys(), np.concatenate((DNA_repair.Gene, np.array(["TGFBR2"]), mock_genes)))]
    if False:
        # DNA types
        print(targets.keys())
        targets = targets[np.intersect1d(targets.keys(),
                          np.array(['BER', 'CSM', 'DDA', 'DRD', 'EPN', 'FA', 'HR', 'MMR', 'NER', 'OTHER', 'POL', 'XPG',
                                    'MOCKFA','MOCKMMR','MOCKPOL','MOCKHR']))]
        print(targets.shape)
    ...
    signature_counts = pd.merge(signatures.T, groups.set_index('sample_id'),
    ...
    target_group = pd.merge(targets.T, groups.set_index('sample_id'),
                                left_index=True, right_index=True, how='inner')\
                        .groupby("group")
    target_counts = pd.merge(target_group.sum().reset_index()\
                                     .melt(id_vars=['group'], value_name='count_tgt', var_name='target'),
                             target_group.count().reset_index()\
    ...
    set_counts['type'] = f"binomial{n},binomial,poisson,binomial"
    ...
    set_counts = set_counts[(set_counts['total_tgt']-set_counts['count_sig']>min_sig_count)]
    print(f"{len(set_counts)} binary tests")
    header = ['signature', 'group', 'target']
    set_counts[header].sort_values(header).to_csv(tests_file_binary, sep="\t", index=False)