...
if __name__=='__main__':
    ...
    filename_output, target_type = sys.argv[1:]
    ...
    # Get total number of Battenberg LoH events
    loh_counts = pd.read_csv(f"{DATA_DIR}/loh_number_size.tsv",
                            sep="\t", index_col=0)
    sample_df = pd.merge(sample_df, loh_counts, how='left',
                         left_on='tumour_sample_platekey', right_index=True)
    sample_df["log_n_loh"] = np.log10(sample_df.n_loh100+1)

    # Get mutation rates
    # Germline
    germline_df = pd.read_csv(f"{DATA_DIR}/aggv2-germline_cancer-gene_hits.tsv",
    ...
    if target_type=='twohit': variables = ['log_age', 'is_female'] + [f"pc{i}" for i in range(1,4)] + ["log_n_loh",]
    elif target_type in ['germline', 'somatic']: variables = ['log_age', 'is_female'] + [f"pc{i}" for i in range(1,4)]
    else: raise ValueError(f"Target type {target_type} unknown")
