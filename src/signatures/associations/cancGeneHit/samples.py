...
if __name__ == "__main__":
    ...
    loh_counts = pd.read_csv(f"{DATA_DIR}/loh_number_size.tsv", sep="\t", index_col=0)
    sample_df = pd.merge(
        sample_df,
        loh_counts,
        how="left",
    ...
    elif target_type in ["germline", "somatic"]:
        variables = ["log_age", "is_female"] + [f"pc{i}" for i in range(1, 4)]
    else:
        raise ValueError(f"Target type {target_type} unknown")
