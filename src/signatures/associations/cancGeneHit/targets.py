...
if __name__ == "__main__":
    ...
    germline_df = (
        pd.read_csv(
            f"{DATA_DIR}/aggv2-germline_cancer-gene_hits.tsv",  # "germline_30/output/aggv2-germline_cancer-gene_hits.tsv",
            sep="\t",
        )
        .set_index("gene_id")
        .T
    ...
    twohit_df = pd.DataFrame(
        np.array(germline_df > 0).astype(int)
    ...
    for type in np.unique(DNA_repair["Type"]):
            ...
            type_hit[
                np.sum(
                    twohit_df[
                        [
                            f"{gene}_{i}"
                            for gene in DNA_repair.Gene[DNA_repair.Type == type]
                        ]
                    ]
                    > 0,
                    axis=1,
        ...
        total_hits = (
            twohit_df[f"{type}_0"] + twohit_df[f"{type}_1"] + twohit_df[f"{type}_2"]
        )
        twohit_df[f"{type}"] = np.where(total_hits > n, 2, total_hits)

    # Generate mock targets - for 1%,5%,20%,50% hit rate
    for hit_rate in [5, 15, 50]:
        twohit_df[f"MOCK{hit_rate:02d}_0"] = scipy.stats.binom.rvs(
            1, hit_rate / 100, size=len(twohit_df)
        )
        twohit_df[f"MOCK{hit_rate:02d}_1"] = scipy.stats.poisson.rvs(
            hit_rate / 100, size=len(twohit_df)
        )
        twohit_df[f"MOCK{hit_rate:02d}_2"] = scipy.stats.binom.rvs(
            1, hit_rate / 100, size=len(twohit_df)
        )
        total_hits = (
            twohit_df[f"MOCK{hit_rate:02d}_0"]
            + twohit_df[f"MOCK{hit_rate:02d}_1"]
            + twohit_df[f"MOCK{hit_rate:02d}_2"]
        )
