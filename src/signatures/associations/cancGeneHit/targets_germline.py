...
if __name__=="__main__":
    ...
    germline_df = pd.DataFrame(np.array(germline_df > 0).astype(int),
    ...
    for type in np.unique(DNA_repair['Type']):
        ...
        type_hit[np.sum(germline_df[[gene for gene in DNA_repair.Gene[DNA_repair.Type==type]]]>0, axis=1)>0] = 1
        germline_df[f"{type}"] = type_hit

    # Generate mock targets - for 1%,5%,20%,50% hit rate
    for hit_rate in [5,15,50]:
        germline_df[f'MOCK{hit_rate:02d}'] = scipy.stats.binom.rvs(1,hit_rate/100,size=len(germline_df))
