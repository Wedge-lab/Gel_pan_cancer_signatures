...
if __name__ == "__main__":
    ...
    for i in range(2):
        targets = pd.read_csv(
            [targets_file, targets_cohort_file][i], sep="\t"
        ).set_index("sample_id")
        groups = pd.read_csv(
            [samples_file, samples_cohort_file][i],
        ...
        signature_counts = pd.merge(
            signature_groups.sum()
            .reset_index()
            .melt(id_vars=["group"], value_name="count_sig", var_name="signature"),
            signature_groups.count()
            .reset_index()
        ...
        set_counts = pd.merge(
            signature_counts,
            pd.merge(
                target_group_0, target_group_1, on=("group", "target"), how="inner"
            ),
            on="group",
            how="inner",
        )
        ...
        set_counts[header].sort_values(header).to_csv(
            [tests_file, tests_cohort_file][i], sep="\t", index=False
        )

        # Test file for non-zero logistic regression
        set_counts = set_counts[
            (set_counts["total"] - set_counts["count_sig"] > min_sig_count)
        ]
        print(f"{len(set_counts)} binary tests")
        header = ["signature", "group", "target"]
        set_counts[header].sort_values(header).to_csv(
            [tests_file_binary, tests_cohort_file_binary][i], sep="\t", index=False
        )
