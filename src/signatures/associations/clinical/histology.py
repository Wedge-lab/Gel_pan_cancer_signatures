...
if __name__ == "__main__":
    ...
    sample_df.drop("group", axis=1).rename({"tumour_tissue": "group"}, axis=1).to_csv(
        samples_cohort_file, sep="\t", index=False
    )
    ...
    # Test groups
    test_groups = list(
        group_sizes.reset_index().set_index("tumour_tissue").loc[polygroup].group
    )
    for i in range(len(bigroup)):
        test_groups += [
            group_sizes.reset_index()
            .set_index("tumour_tissue")
            .loc[bigroup[i]]
            .group.iloc[0],
        ]
    ...
    sample_df[["sample_id"] + test_groups].to_csv(
        targets_cohort_file, sep="\t", index=False
    )
