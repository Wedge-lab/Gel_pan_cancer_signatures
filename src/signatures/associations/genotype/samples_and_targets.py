...
if __name__=="__main__":
    ...
    sample_df = sample_df[ ( ~(sample_df.tumour_tissue.map(lambda x: x in ["Ovary","Uterus"])&(sample_df.is_female==-1)) ) &\
    ...
    # Get genotype grid
    genotype_df = pd.read_csv(f"{DATA_DIR}/pancancer_mutationTimeR_updated_CNACalls_2023_09_16.tsv",
                                usecols=['participant_id', 'tumour_sample_platekey', 'germline_sample_platekey', 'is_WGD'], sep="\t") # 'ploidy',
    # genotype_df['ploidy'] = (genotype_df.ploidy>2.5).astype(int)
    genotype_df['is_WGD'] = genotype_df.is_WGD.astype(int)
    genotype_df['sample_id'] = genotype_df.participant_id.astype(str)+"_"+genotype_df.tumour_sample_platekey+"_"+genotype_df.germline_sample_platekey
    genotype_df = genotype_df.drop(np.intersect1d(genotype_df.keys(),
                                                    ['participant_id', 'tumour_sample_platekey', 'germline_sample_platekey',
                                                     'day_sampling', 'sample_platekey', 'tumour_pseudo_id']), axis=1)
    ...
    for cohort in os.listdir(sv_dir):
        if cohort!='README.md':
            sv_counts = pd.concat((sv_counts,
                                pd.read_csv(f"{sv_dir}/{cohort}/summaryLevel/sampleClusterClassCounts.tsv", sep="\t")))
    sv_counts['sample_id'] = sv_counts.participant_id.astype(str)+"_"+sv_counts.tumour_sample_platekey+"_"+sv_counts.germline_sample_platekey
    ...
    for cluster_type in cluster_types:
        sv_counts[re.sub("^n_", "is_", cluster_type)] = (sv_counts[cluster_type]>0).astype(int)
    genotype_df = pd.merge(genotype_df,
                           sv_counts[['sample_id']+cluster_types+[re.sub("^n_", "is_", cluster_type) for cluster_type in cluster_types]],
                           on='sample_id', how='inner')

    # Get the number of Kataegis events
    ktg_dir = f"{DATA_DIR}/kataegis_analysis/results/SigProfilerClusters_persample/summary/"
    ktg_counts = pd.read_csv(f"{ktg_dir}/All_10983_clustered_events_table.tsv", sep="\t",usecols=['sample_id', 'kataegis'])
    ...
    for cluster_type in cluster_types:
        ktg_counts[re.sub("^n_", "is_", cluster_type)] = (ktg_counts[cluster_type]>0).astype(int)
    genotype_df = pd.merge(genotype_df,
                           ktg_counts[['sample_id']+cluster_types+[re.sub("^n_", "is_", cluster_type) for cluster_type in cluster_types]],
                           on='sample_id', how='inner')
