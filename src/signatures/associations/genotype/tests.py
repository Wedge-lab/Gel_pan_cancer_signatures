...
if __name__=="__main__":
    ...
    types = pd.DataFrame.from_dict({key: "binomial" if key[:3]=='is_' else "poisson" if key[:2]=='n_' else '' \
                                    for key in set_counts.target.unique()},
                                   orient='index', columns=['type'])
    print(types)
    set_counts = pd.merge(set_counts, types, left_on='target', right_index=True, how='inner')
