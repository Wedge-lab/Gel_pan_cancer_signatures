...
if __name__=="__main__":
    ...
    treatment_df = treatments.getTreatment2(sample_df[['participant_id', 'tumour_sample_platekey', 'germline_sample_platekey', 'sample_id',
                                                       'day_sampling']], base="").reset_index()
    treatment_df.rename(columns = dict(zip(treatment_df.keys()[3:], [x.replace(' ','_') for x in treatment_df.keys()[3:]])),
                        inplace=True)
    treatment_df = treatment_df.drop(np.intersect1d(treatment_df.keys(),
    ...
    for hit_rate in [10,5,1]:
        treatment_df[f'MOCK{hit_rate:03d}'] = scipy.stats.binom.rvs(n,hit_rate/1000,size=len(treatment_df))
    # Generate mock targets - perturbation of genes within cohorts
    for treat in ["CARBOPLATIN", "FLUOROURACIL", "RADIOTHERAPY"]: #"rtprescribeddose"]:
        indices = np.arange(len(treatment_df))
        MOCKtreat = np.zeros(len(treatment_df), dtype=int)
        for group in np.unique(X.group):
            MOCKtreat[X.group==group] = treatment_df[treat][np.random.choice(indices[X.group==group], size=np.sum(X.group==group), replace=False)]
