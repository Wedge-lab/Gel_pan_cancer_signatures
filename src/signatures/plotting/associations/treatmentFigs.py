...
from signatures.plotting.combinedSignatures import loadSignatures, signatureRenamer, publish_fig
...
if __name__=='__main__':
    ...
    treatment_list = {'rtprescribeddose': 'Radiotherapy', 'RADIOTHERAPY': 'Radiotherapy',
                ...
                'BEVACIZUMAB': 'BEVACIZUMAB', 'BOSUTINIB': 'BOSUTINIB',
                'CAPECITABINE': 'CAPECITABINE', 'CETUXIMAB': 'CETUXIMAB',
                'DASATINIB': 'DASATINIB', 'DOCETAXEL': 'DOCETAXEL', 'DOXORUBICIN': 'DOXORUBICIN',
                'FOLINIC_ACID': 'FOLINIC ACID', 'GEMCITABINE': 'GEMCITABINE', 'HYDROXYCARBAMIDE': 'HYDROXYCARBAMIDE',
                'IFOSFAMIDE': 'IFOSFAMIDE', 'IMATINIB': 'IMATINIB', 'IRINOTECAN': 'IRINOTECAN',
                'PACLITAXEL': 'PACLITAXEL', 'PEMETREXED': 'PEMETREXED', 'PERTUZUMAB': 'PERTUZUMAB',
                'TEMOZOLOMIDE': 'TEMOZOLOMIDE', 'TRASTUZUMAB': 'TRASTUZUMAB'}
    treatment_list = {key:treatment_list[key][0].upper()+treatment_list[key][1:].lower() for key in treatment_list}

    # Input data
    sample_df = pd.read_csv(f"{results_dir}/{run_name}/input/samples.tsv", sep="\t").set_index('sample_id')
    target_df = pd.read_csv(f"{results_dir}/{run_name}/input/targets.tsv", sep="\t").set_index('sample_id')
    test_df = pd.read_csv(f"{results_dir}/{run_name}/input/tests.tsv", sep="\t")
    signature_df = pd.read_csv(f"{results_dir}/{run_name}/input/signatures.tsv", sep="\t").set_index('sample_id')
    ...
    results_mock = results_combined[(mock_match(results_combined.target))|\
                                    (mock_match(results_combined.signature))].copy()
    ...
    results_combined = pd.merge(results_combined,
    ...
    for key,combos in pos_sets.items():
        for i,set_row in enumerate(combos):
            print(set_row)
            rset = results_combined[subset&(results_combined[key.split(",")[0]]==set_row[0])&\
    ...
    ellipse = Ellipse((5.2,38), width=3.6, height=42, facecolor='none', edgecolor='grey', linestyle="--")
    ...
    subsets = [subset&(results_mock.signature=="SBSmock"),
    ...
    for i, subset in enumerate(subsets):
        ...
        plt.scatter(-np.log10(percentiles[1][::-1]), Y,
                    s=20, label=f"dCRT",
                    c=default_colours[0], marker=".")#, marker=markers[i])
        plt.scatter(-np.log10(percentiles[1][::-1])[Y>ymax], np.zeros(np.sum(Y>ymax))+ymax-0.1,
                    s=20, c=default_colours[0], marker="^")
    ...
    tgt_df.replace({"dependent":treatment_list, "target":treatment_list}, inplace=True)
    ...
    target_indices = np.intersect1d(target_subset, list(treatment_list.values()), return_indices=True)
    ...
    for i,group in enumerate(groups):
        ...
        p_threshold = np.max(np.array(tgt_df.wilks_pvalue)[BenjiminiHochberg(np.array(tgt_df.wilks_pvalue),0.01)])

        tgt_subset = tgt_df.group==group
        target_grid = correlationGrid(tgt_df[tgt_df.group==group],
                                    target_subset,
                                    0.01)[target_subset].loc[target_subset]
        ...
        axes[i].set_xticklabels(target_subset, rotation=90, fontsize=fs*1.2, minor=False);
    ...
    new_grid = pd.merge(sample_df[['group']], target_df, left_index=True, right_index=True, how='left')

    treatments_ = new_grid.keys()[7:]
    treatments_ = treatments_[~mock_match(treatments_)]
    treatment_xm = np.intersect1d(treatments_, list(treatment_list.values()), return_indices=True)
    ...
    fig, ax = plt.subplots(1,1,figsize=(12,12*np.sum(subset)/count_grid.shape[1]))


    im = plt.pcolor(count_grid[subset][::-1], cmap='inferno_r', norm=LogNorm(),
                    vmin=5, vmax=100)

    low_count_idx = np.unravel_index(np.argwhere(np.array(low_count)[subset][::-1]),
                                    np.array(low_count)[subset][::-1].shape)[1]
    plt.scatter(low_count_idx[:,1]+0.5, low_count_idx[:,0]+0.5, marker='o', color='k', s=10)
    ...
    legend_elements = [Line2D([0], [0], marker='o', color='w', label=f"$\mathrm{{count}} \leq 5$",
                    markerfacecolor='k', markersize=10)]
    plt.legend(handles=legend_elements, bbox_to_anchor=(1.,1.), loc="lower right", fontsize=20, frameon=False);
