...
if __name__=='__main__':
    ...
    results_zinb = pd.read_csv(
        f"{run_dir}/output/signature_target_assoc_nb.csv"
    )
    results_log0 = pd.read_csv(
        f"{run_dir}/output/signature_target_assoc_logistic.csv"
    )
    results_target = pd.read_csv(
        f"{run_dir}/output/target_target_assoc.csv"
    )
    ...
    results_combined['pv_cov'] = (results_combined['target_means_alt_zinb'])**2/\
                                scipy.stats.chi2.isf(results_combined['resample_pvalue'], df=1)

    results_combined['pv_IV'] = 1/results_combined['pv_cov']
    results_combined['pv_IVWmean'] = results_combined['target_means_alt_zinb']*results_combined['pv_IV']
    results_groupsum = results_combined[['signature', 'target', 'pv_IV', 'pv_IVWmean']].groupby(['target','signature']).sum()
    results_groupsum['target_agg_means_alt_zinb'] = results_groupsum['pv_IVWmean']/results_groupsum['pv_IV']
    results_groupsum['target_agg_covs_alt_zinb'] = 1/results_groupsum['pv_IV']
    results_groupsum['target_agg_z_alt_zinb'] = results_groupsum['target_agg_means_alt_zinb']*np.sqrt(results_groupsum['pv_IV'])
    ...
    unique_signatures = {mut_type: [sig for sig in unique_signatures if sig[:len(mut_type)]==mut_type] \
    ...
    results_combined = pd.merge(results_combined,
                                ...
                                on=['target', 'signature'], how='inner')
    significant_results_count = results_combined[
                    (results_combined['resample_pvalue']<=pv_sws)&\
                    (results_combined['target_means_alt_zinb']*results_combined['target_agg_z_alt_zinb'])>0].groupby(['target', 'signature']).size()



    fig, axes = plt.subplots(len(unique_signatures), 5,figsize=(10,18),
                            gridspec_kw={'height_ratios':[len(unique_signatures[mut_type]) for mut_type in unique_signatures][::-1]},
                            sharex='col', sharey='row')
    plt.subplots_adjust(hspace=0.02,wspace=0.02)
    genotype_labels = {'is_WGD':'WGD',
    ...
    discrete_cmap = mpl.colors.ListedColormap(cm.viridis_r(np.linspace(0,1,max_cohort+1)))
    ...
    for j,genotype in enumerate(genotype_labels):
        ...
        for i,mut_type in enumerate(unique_signatures.keys()):
            ...
            results_genotype = pd.merge(results_genotype,
                                pd.DataFrame(index=unique_signatures[mut_type]),
                                left_index=True, right_index=True, how='right')

            ax = axes[4-i, j]
            plt.sca(ax)
            colors = mapper.to_rgba(pd.merge(pd.DataFrame(significant_results_count.loc[genotype]),
                                            pd.DataFrame(index=unique_signatures[mut_type]),
                                left_index=True, right_index=True, how='right').fillna(0)[0])
            z_scores = results_genotype.loc[unique_signatures[mut_type],'target_agg_z_alt_zinb']
            out_of_bounds = np.abs(z_scores)>z_max
            im=plt.scatter(z_scores[~out_of_bounds],
                        np.arange(len(unique_signatures[mut_type]))[~out_of_bounds],
                        c=colors[~out_of_bounds], s=15, zorder=0)
            plt.scatter(np.sign(z_scores)[out_of_bounds&(z_scores>0)]*(z_max*1.03),
                        np.arange(len(unique_signatures[mut_type]))[out_of_bounds&(z_scores>0)],
                        c=colors[out_of_bounds&(z_scores>0)], s=15, zorder=0,
                        marker=">")
            plt.scatter(np.sign(z_scores)[out_of_bounds&(z_scores<0)]*(z_max*1.03),
                        np.arange(len(unique_signatures[mut_type]))[out_of_bounds&(z_scores<0)],
                        c=colors[out_of_bounds&(z_scores<0)], s=15, zorder=0,
            ...
            if j==0:
                sig_type = {'SBS':'SBS288', 'DBS':'DBS78', 'ID':'ID83', 'CN':'CNV48', 'SV':'SV32'}[mut_type]
