...
def survival(sample_df, target, covariates, transformation='lognorm'):
    ...
    logrank=logrank_test(np.array(sample_df[target_arr==0].survival_time),
    ...
    kmf.fit(durations = np.array(sample_df[subset_split].survival_time),
    ...
    try:
        ...
        if transformation=='lognorm':
            ...
            cph_df[target] = (cph_df[target]-np.mean(cph_df[target]))/np.std(cph_df[target])

        # Cox proportional hazard
        cph_alt = CoxPHFitter()
        alt_df = cph_df[['survival_time','vital_status']+covariates+[target,]]
...
if __name__=='__main__':
    ...
    # Import sample data
    sample_df = pd.read_csv("/re_gecip/shared_allGeCIPs/pancancer_signatures/results/survival/survival_log_grade-4_unique/input/samples.tsv",
                            sep="\t", index_col=0)
    sample_df.drop_duplicates(inplace=True)
    sample_df = sample_df[~np.isnan(sample_df.grade)]
    covariates = [key for key in sample_df.keys() if not key in ['group', 'survival_time', 'vital_status']]

    # Add tumour type
    tumour_type = pd.read_csv("/re_gecip/shared_allGeCIPs/pancancer_signatures/results/sample_lists_incl_SEGs/sample_list_2021_06_29.tsv",
                                sep="\t", usecols=['participant_id', 'tumour_sample_platekey', 'germline_sample_platekey', 'tumour_type'])
    tumour_type['sample_id'] = tumour_type.participant_id.astype(str)+'_'+tumour_type['tumour_sample_platekey']+'_'+tumour_type['germline_sample_platekey']
    sample_df = pd.merge(sample_df, tumour_type, on='sample_id', how='inner').set_index('sample_id')

    # Import tgtnature data
    target_df = pd.read_csv("/re_gecip/shared_allGeCIPs/pancancer_signatures/results/survival/survival_log_grade-4_unique/input/signatures.tsv", sep="\t", index_col=0)
    ...
    subset_df = pd.merge(sample_df[subset], target_df, how='inner', left_index=True, right_on='sample_id')\
                        .sort_values('survival_time')

    # Add clinical data
    clinical_data = pd.read_csv("/re_gecip/shared_allGeCIPs/pancancer_signatures/results/associations/clinicalSigs/clinical_tumour-group_InDelQC_nb/input/targets.tsv",
                                sep="\t", index_col=0)
    clinical_data = clinical_data.loc[subset_df.index]
    subset_df = pd.merge(subset_df, clinical_data[['pr_status', 'er_status', 'her2_status']],
                        left_index=True, right_index=True, how='inner')

    subset_df['HRD'] = (((subset_df['SBS3']>0).astype(int) \
                        + (subset_df['ID6']>0).astype(int) \
                        + (subset_df['CN17']>0).astype(int)) > 1).astype(int)
    ...
    for tgt in ['er_status', 'pr_status', 'her2_status']:
        ...
        cph_alt, logrank = survival(subset_df[subset], tgt, covariates, transformation='none')
