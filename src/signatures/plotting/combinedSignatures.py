...
from signatures.combineSignatures._03_combineSignatures import (
    cosineSimilarity,
    orderSignatures,
)
...
def loadSignatures(sv_rename_deg=False):
    ...
    for sig_type in sig_dirs:
        combined_sigs[sig_type] = pd.read_csv(
            f"{sig_dirs[sig_type]}/Combined_Solution_Signatures.tsv",
            sep="\t",
            index_col=0,
        )
        combined_acts[sig_type] = pd.read_csv(
            f"{sig_dirs[sig_type]}/Combined_Solution_Activities.tsv",
            sep="\t",
            index_col=0,
        ).fillna(0)
    ...
    reference_sigs = {
        sig_type: pd.read_csv(references[sig_type], sep="\t", index_col=0)
    ...
    max_sigs = {
        sig_type: np.max(
            np.array(
                [
                    regexSearch("([A-Z]+)([0-9]+)([a-z]*)", sig, 2)
                    for sig in reference_sigs[sig_type].keys()
                ]
            ).astype(int)
        )
        for sig_type in combined_sigs
    ...
    for sig_type in reference_sigs:
        reference_map[sig_type] = iterativeMapping(
            combined_sigs[sig_type],
            reference_sigs[sig_type],
            mut_type=regexSearch("([A-Z]+)[0-9]+", sig_type, 1).replace("CNV", "CN"),
        )  # type: ignore

    # Change SV6 to SV6a as we will also have an SV6b from Degasperi
    if sv_rename_deg:
        reference_map["SV32"]["new"] = reference_map["SV32"].new.replace(
            {"SV6": "SV6a"}
        )
    ...
    for sig_type in degasperi_sigs:
        degasperi_sigs[sig_type] = degasperi_sigs[sig_type].loc[
            combined_sigs[sig_type].index
        ]

    # Replace signature labels from COSMIC
    mapped_sigs = {}
    for sig_type in combined_sigs:
        combined_acts[sig_type].rename(
            reference_map[sig_type].set_index("old")["new"].to_dict(),
            axis=1,
            inplace=True,
        )
        combined_sigs[sig_type].rename(
            reference_map[sig_type].set_index("old")["new"].to_dict(),
            axis=1,
            inplace=True,
        )
    ...
    if sv_rename_deg:
        degasperi_sv_map = iterativeMapping(
            combined_sigs["SV32"].drop(reference_map["SV32"].old, axis=1),
            degasperi_sigs["SV32"],
            mut_type=regexSearch("([A-Z]+)[0-9]+", "SV32", 1),
        )

        combined_acts["SV32"].rename(
            degasperi_sv_map.set_index("old")["new"].to_dict(), axis=1, inplace=True
        )
        combined_sigs["SV32"].rename(
            degasperi_sv_map.set_index("old")["new"].to_dict(), axis=1, inplace=True
        )
    ...
    for sig_type in combined_sigs:
        ...
        novel_map[sig_type] = (
            pd.DataFrame.from_dict(novel_map[sig_type], orient="index")
            .drop("X")
            .reset_index()
    ...
    for sig_type in novel_map:
        combined_acts[sig_type].rename(
            novel_map[sig_type].set_index("old")["new"].to_dict(), axis=1, inplace=True
        )
        combined_sigs[sig_type].rename(
            novel_map[sig_type].set_index("old")["new"].to_dict(), axis=1, inplace=True
        )
    ...
    degasperi_map = {}
    for sig_type in degasperi_sigs:
        degasperi_map[sig_type] = iterativeMapping(
            combined_sigs[sig_type].drop(reference_map[sig_type].new, axis=1),
    ...
    return (
        combined_acts,
        combined_sigs,
        {"reference": reference_map, "degasperi": degasperi_map, "novel": novel_map},
    )
...
def signatureRenamer(relabel_map, sv_rename_deg=False):
    ...
    if sv_rename_deg:
        rename_df = pd.concat((rename_df, relabel_map["degasperi"]["SV32"]))
...
def iterativeMapping(signatures, reference, threshold=0.8, mut_type="SV"):
    ...
    cossim_mtx = cosineSimilarity(
        np.array(signatures[np.sort(signatures.keys())]),
        np.array(reference[np.sort(reference.keys())]),
    )
    mapping = {}
    while (len(nov_sig_list) > 0) & (len(ref_sig_list) > 0):
        best_match = np.unravel_index(
            np.argmax(cossim_mtx), (len(nov_sig_list), len(ref_sig_list))
        )
    ...
    mapping = {
        key: mut_type + regexSearch("[A-Za-z]+([0-9]+[a-z]*)", val, 1)
...
def plotSignature(
    ...
    if mutation_type == "SBS":
        context_1 = signature.index.map(
            lambda x: regexSearch("([A-Z])\[([A-Z>]+)\]([A-Z])", x, 1)
        )
        snv = signature.index.map(
            lambda x: regexSearch("([A-Z])\[([A-Z>]+)\]([A-Z])", x, 2)
        )
        context_2 = signature.index.map(
            lambda x: regexSearch("([A-Z])\[([A-Z>]+)\]([A-Z])", x, 3)
        )
        order = np.argsort(
            np.unique(snv, return_inverse=True)[1] * 100
            + np.unique(context_1, return_inverse=True)[1] * 10
            + np.unique(context_2, return_inverse=True)[1]
        )
        snv = snv[order]
        context_1 = context_1[order]
        context_2 = context_2[order]
        ...
        colors = np.repeat(
            np.array(["deepskyblue", "k", "red", "silver", "limegreen", "pink"]),
            type_counts,
        )
        plt.xlim(-1, 96)
        label_freq = 1  # 4
        ...
        super_type = signature.index.map(
            lambda x: regexSearch("([A-Z]+)>([A-Z]+)", x, 1) + ">"
        )
        ...
        colors = np.repeat(
            plt.rcParams["axes.prop_cycle"].by_key()["color"][: len(type_counts)],
            type_counts,
        )
        colors = np.repeat(
            np.array(
        ...
        colors = np.repeat(
            np.array(
                ["darkorange", "forestgreen", "firebrick", "deepskyblue", "darkviolet"]
            ),
            type_counts,
        )
        label_freq = 1  # 4
        ...
        raise ValueError("mutation_type unknown")
    ...
    label_counts = label_counts[
        np.argsort(np.intersect1d(super_type, label_unique, return_indices=True)[1])
    ]
    if xticks:
        ax.xaxis.set_tick_params(
            which="both",
            labelbottom=True,
            labelsize=fs * 0.6 * (100 / len(mutation_labels)) ** 0.15,
            rotation=90,
        )
        ...
        for i1 in np.cumsum(label_counts) - label_counts / 2:
            plt.text(
                i1 - 0.5,
                -pad * np.diff(ax.get_ylim()),
                rf"{super_type[int(i1)]}",
                ha="center",
                va="top",
                rotation=90,
                fontsize=fs * 0.8,
    ...
    ylim = ax.get_ylim()
    for posx in np.cumsum(type_counts)[:-1]:
        plt.plot(
            [
                posx - 0.5,
            ]
...
def plotNovelSignatures(combined_sigs, relabel_map, sv_rename_deg=False):
    ...
    width_ratios = [
        [len(relabel_map["novel"][sig_type]) + 1 for sig_type in plot_types[:3]],
        [len(relabel_map["novel"]["CNV48"]) + 1, len(combined_sigs.keys()) + 1],
    ]
    for j in range(2):
        width_ratios[j] = np.insert(
            (np.cumsum(width_ratios[j]) / np.sum(width_ratios[j]) * hpixels).astype(
                int
            ),
            0,
            0,
    ...
    gs = fig.add_gridspec(
        nrows=hpixels, ncols=100, left=0.05, right=0.48, wspace=0.1, hspace=0.1
    )
    ...
    for i, sig_type in enumerate(plot_types):
        ...
        if (sig_type == "SV32") & sv_rename_deg:
            signatures = np.hstack(
                (
                    relabel_map["reference"][sig_type].new,
                    relabel_map["degasperi"][sig_type].old,
                    relabel_map["novel"][sig_type].new,
                )
            )
        elif sig_type == "SV32":
            signatures = np.hstack(
        ...
        for isig, sig in enumerate(signatures):
            ...
            if j == 0:
                ax = fig.add_subplot(
                    gs[
                        int(hmin + isig * (hmax - hmin) / len(signatures)) : hmin
                        + int((isig + 1) * (hmax - hmin) / len(signatures))
                        - 1,
                        0:55,
                    ]
                )
            ...
            plt.text(
                ...
                va="top",
                fontsize=24,
                color=map_colors["degasperi"]
                if sig in list(relabel_map["degasperi"][sig_type].old)
                else map_colors["novel"]
                if sig in list(relabel_map["novel"][sig_type].new)
                else "k",
            )
            if isig != len(signatures) - 1:
                ax.set_xticklabels([])
...
def groupSignatureActivities(sample_df, combined_acts, cohorts="tumour_group"):
    ...
    for sig_type in combined_acts:
        ...
        acts = pd.merge(
            sample_df[["sample_platekey", cohorts]],
            acts,
            left_on="sample_platekey",
            right_index=True,
        )
        ...
        # Make plot grids:
        tables["count"][sig_type] = np.zeros(
            (len(signatures[sig_type]), len(unique_tissues)), dtype=int
        )
        tables["median"][sig_type] = np.zeros(
            (len(signatures[sig_type]), len(unique_tissues))
        )

        for isig, signature in enumerate(signatures[sig_type]):
            for it, tissue in enumerate(unique_tissues):
                tables["count"][sig_type][isig, it] = np.sum(
                    acts[signature][acts[cohorts] == tissue] > 0
                )
                tables["median"][sig_type][isig, it] = np.median(
                    acts[signature][acts[cohorts] == tissue][
                        acts[signature][acts[cohorts] == tissue] > 0
                    ]
                )
...
def plotCohortActivities(
    tables, signatures, orientation="horizontal", rename_sigs=None
):
    """
    ...
    width_ratios = [
        np.sum(np.sum(tables["count"][sig_type], axis=1) > 0)
        for sig_type in tables["count"]
    ]
    width_ratios = np.insert(
        (np.cumsum(width_ratios[1:]) / np.sum(width_ratios[1:]) * 300).astype(int), 0, 0
    )
    width_ratios[0] -= 2
    width_ratios[-1] += 2
    ...
    if orientation == "horizontal":
        ...
        gs = fig.add_gridspec(
            nrows=2, ncols=300, left=0.05, right=0.48, wspace=0.1, hspace=0.4
        )
    else:
        fig = plt.figure(constrained_layout=False, figsize=(35, 30))
        gs = fig.add_gridspec(
            nrows=300, ncols=2, left=0.05, right=0.48, wspace=0.3, hspace=0.1
        )
    ...
    unique_tissue_labels = [
        tissue + f"({str(tables['size'].loc[tissue])})".rjust(7)
    ...
    for i, sig_type in enumerate(tables["count"].keys()):
        if i == 0:
            ax = (
                ...
                if orientation == "horizontal"
                else fig.add_subplot(gs[:, 0])
            )
        else:
            ax = (
                fig.add_subplot(gs[1, width_ratios[i - 1] + 3 : width_ratios[i] - 3])
                if orientation == "horizontal"
                else fig.add_subplot(
                    gs[width_ratios[i - 1] + 2 : width_ratios[i] - 2, 1]
                )
            )

        sig_subset = np.sum(tables["count"][sig_type], axis=1) > 0

        xx_sig, yy_tis = np.meshgrid(
            np.arange(len(unique_tissues)), np.arange(np.sum(sig_subset))
        )
        ...
        if orientation == "horizontal":
            ...
            ax.set_xticks(
                np.arange(len(signatures[sig_type][sig_subset]) + 1), minor=True
            )
            ax.grid(
            ...
            ax.set_xticks(
                np.arange(len(signatures[sig_type][sig_subset])) + 0.5, minor=False
            )
            ax.set_yticks(np.arange(len(unique_tissues)) + 0.5, minor=False)
            ax.set_xticklabels(
                signatures[sig_type][sig_subset], fontsize=16, minor=False, rotation=90
            )

            if i < 2:
                ax.set_yticklabels(
                    [label.replace("_", " ") for label in unique_tissue_labels],
                    fontsize=16,
                    minor=False,
                )
            else:
            ...
            im = ax.scatter(
                ...
                yy_tis.flatten() + 0.5,
                s=size_factor(
                    (100 * count_table[::-1] / group_count[None, :]).flatten()
                ),
                cmap="inferno_r",
                norm=LogNorm(vmin=1, vmax=vmax),
            ...
            ax.set_yticklabels(
                signatures[sig_type][sig_subset], fontsize=16, minor=False, rotation=0
            )

            if (i == 0) | (i == 4):
                ax.set_xticklabels(
                    [label.replace("_", " ") for label in unique_tissues],
                    fontsize=16,
                    minor=False,
                    rotation=90,
                )
        ...
        for isig, sig in enumerate(signatures[sig_type][sig_subset]):
            if rename_sigs is not None:
                if sig in list(rename_sigs["degasperi"][sig_type].old):
                    ax.get_xticklabels()[isig].set_color(
                        map_colors["degasperi"]
                    ) if orientation == "horizontal" else ax.get_yticklabels()[
                        isig
                    ].set_color(map_colors["degasperi"])
                elif sig in list(rename_sigs["novel"][sig_type].new):
                    ax.get_xticklabels()[isig].set_color(
                        map_colors["novel"]
                    ) if orientation == "horizontal" else ax.get_yticklabels()[
                        isig
                    ].set_color(map_colors["novel"])
            else:
...
def fisherTests(combined_act_df):
    signature_present = (
        (np.array(combined_act_df) > 0)
        & (np.array(combined_act_df) > np.median(combined_act_df, axis=0))
    ).astype(int)
    ...
    fisher_test = pvalue_npy(
        contingency[:, :, 0, 0].flatten().astype(np.uint64),
    ...
    for i in tqdm.tqdm(range(contingency.shape[0]), total=contingency.shape[0]):
        for j in range(i + 1):
            fisher_test = scipy.stats.fisher_exact(
                contingency[i, j], alternative="two-sided"
            )
...
def clusterHierarchical(combined_act_df):
    ...
    clusters = shc.linkage(
        pd.DataFrame(X.T, index=combined_act_df.keys()[~nan_subset]),
        method="ward",
        metric="euclidean",
    )
...
def cluster_diagram(
    ...
    fs=12,
    use_cbar=True,
    cmap_sp=plt.get_cmap("bwr"),
    cmap_f=plt.get_cmap("PRGn"),
):
    plt.sca(ax)
    ...
    full_mtx = (
        np.tril(spearman_stat / 2 + 0.5)
        - np.eye(np.sum(spearman_stat.shape[0]))
        + np.triu((fisher_grid + PVMAX) / (2 * PVMAX) + 1)
    )
    full_mtx[np.arange(full_mtx.shape[0]), np.arange(full_mtx.shape[0])] = 1 - 1e-10

    # sample the colormaps that you want to use. Use 128 from each so we get 256
    # colors in total
    colors1 = cmap_sp(squish(np.linspace(0.0, 1, 127), s=0.2))
    colors2 = cmap_f(
        np.linspace(0.1, 0.9, 128)
    )  # plt.plt.get_cmap('bone_r')(np.linspace(0., 1, 128))
    colors2[
    ...
    if use_cbar:
        cax = fig.add_axes(
            [
                ...
                ax.get_position().y0 + ax.get_position().height * 0.02,
                0.02,
                ax.get_position().height * 0.46,
            ]
        )
        cbar = fig.colorbar(
            cm.ScalarMappable(
                norm=Normalize(vmin=-PVMAX, vmax=PVMAX),
        ...
        cbar.set_label(
            rf"$\pm\log_{{10}}\left(\mathrm{{Fisher}}\,\,P-\mathrm{{value}}\right)$",
            fontsize=fs * 2,
        )
        ...
        cax2 = fig.add_axes(
            [
                ax.get_position().x1 + 0.01,
                ax.get_position().y0 + ax.get_position().height * 0.52,
                0.02,
                ax.get_position().height * 0.46,
            ]
        )
        cbar = fig.colorbar(
            cm.ScalarMappable(norm=Normalize(vmin=-1, vmax=1), cmap=map1),
            cax=cax2,
            orientation="vertical",
        )
        cbar.ax.tick_params(labelsize=fs * 1.5)
...
def fullClusterDiagram(
    combined_act_df, fisher_stat, fisher_pv, spearman_stat, spearman_pv
):
    ...
    fig, axes = plt.subplots(
        2, 1, figsize=(27, 30), gridspec_kw={"height_ratios": [1, 9]}
    )
    ...
    intersection = np.intersect1d(
        dendro["ivl"], combined_act_df.keys()[~nan_subset], return_indices=True
    )
    ...
    ax.set_xticklabels(
        combined_act_df.keys()[~nan_subset][order],
        rotation=90,
        fontsize=fs,
        minor=False,
    )
    ...
    ax.set_yticklabels(
        combined_act_df.keys()[~nan_subset][order], fontsize=fs, minor=False
    )
    ...
    for label, gene_group in group_labels.items():
        ...
        plt.text(
            np.mean(intersection[1]) + 0.5,
            -10,
            label,
            ha="center",
            va="center",
            fontsize=fs * 1.5,
            color=default_colours[colour_id[intersection[1]][0] + 1],
            rotation=30,
        )
...
def clusterGroups(combined_act_df, fisher_stat, fisher_pv, spearman_stat, spearman_pv):
    ...
    group_labels = {
        ...
        "Clustered Rearrangments": ["SV4", "SV6", "SV13"],
    }  # ,"SV10","CN18","DBS6"]}

    fig, axes = plt.subplots(3, 2, figsize=(16, 24))

    dendro = shc.dendrogram(
        Z=clusters, labels=combined_act_df.keys()[~nan_subset], no_plot=True
    )
    ...
    for i, group in enumerate(
        ["UV", "Smoking", "MMR", r"$POLE$", "HRD", r"$POLG$"]
    ):  #'Clock 1']):
        subset_indices = np.intersect1d(
            group_labels[group], combined_act_df.keys(), return_indices=True
        )[2]
        subset_labels = np.array(
            combined_act_df.keys()[
                np.sum(
                    (fisher_pv[subset_indices] < pv_sws)
                    & (fisher_stat[subset_indices] > 1),
                    axis=0,
                )
        ...
        subset_indices = np.intersect1d(
            subset_labels, combined_act_df.keys(), return_indices=True
        )
        ...
        fs = 16
        plt.sca(ax)
        colors1, colors2 = cluster_diagram(
            fig,
            ax,
        ...
        ax.set_xticks(np.arange(len(order)) + 0.5, minor=False)
        ax.set_xticklabels(
            np.array(subset_labels)[order], rotation=90, fontsize=fs, minor=False
        )
        ax.set_yticks(np.arange(len(order)) + 0.5, minor=False)
        ax.set_yticklabels(np.array(subset_labels)[order], fontsize=fs, minor=False)
        colour_id = np.unique(dendro["leaves_color_list"], return_inverse=True)[1][
            np.sort(intersection[1])
        ]
        for i, col_i in enumerate(colour_id):
            ax.get_xticklabels()[i].set_color(default_colours[col_i + 1])
            ax.get_yticklabels()[i].set_color(default_colours[col_i + 1])
    ...
    cax = fig.add_axes(
        [
            ax.get_position().x1 + 0.01,
            ax.get_position().y0 - ax.get_position().height * 1.1,
            0.02,
            ax.get_position().height * 1.5,
        ]
    )
    cbar = fig.colorbar(
        plt.get_cmap("ScalarMappable")(
            norm=Normalize(vmin=-PVMAX, vmax=PVMAX),
            cmap=mcolors.LinearSegmentedColormap.from_list("my_colormap", colors2),
        ),
        cax=cax,
    ...
    cax2 = fig.add_axes(
        [
            ax.get_position().x1 + 0.01,
            ax.get_position().y0 + ax.get_position().height * 0.6,
            0.02,
            ax.get_position().height * 1.5,
        ]
    )
...
def clusterSets(
    ...
    for i, group in enumerate(groups):  #'Clock 1']):
        # Reorder according to Dendrogram
        intersection = np.intersect1d(
            dendro["ivl"], group_labels[group], return_indices=True
        )

        # Get groups which are included and all of their signatures
        dendro_colors = np.unique(
            np.array(dendro["leaves_color_list"])[intersection[1]]
        )
        subset_labels = np.array(dendro["ivl"])[
            np.in1d(dendro["leaves_color_list"], dendro_colors)
        ]
        subset_indices = np.intersect1d(
            subset_labels, combined_act_df.keys(), return_indices=True
        )
...
def getMatrices(sample_df):
    ...
    cohorts = [
        ...
        for folder in os.listdir(
            "/re_gecip/cancer_pan/fprefect/botl/results/SIGmats/v3/matrices/"
        )
    ...
    for mut_type in matrices:
        ...
        for i, cohort in enumerate(cohorts):
            cohort_matrix = pd.read_csv(
                f"/re_gecip/cancer_pan/fprefect/botl/results/SIGmats/{drafts[mut_type]}/{matrices[mut_type]}/"
            ...
            if i == 0:
                ...
                sig_mats[matrices[mut_type]] = pd.merge(
                    sig_mats[matrices[mut_type]],
                    cohort_matrix,
                    left_index=True,
                    right_index=True,
                    how="inner",
        ...
        if mut_type == "ID":
            sig_mats["ID83"] = (
                pd.merge(
                    sig_mats["ID83"],
                    sample_df[["tumour_sample_platekey", "sample_platekey"]],
                    left_index=True,
                    right_on="tumour_sample_platekey",
                    how="inner",
                )
                .drop("tumour_sample_platekey", axis=1)
                .set_index("sample_platekey")
            )
        sig_totals = pd.merge(
            sig_totals,
            pd.DataFrame(
                np.sum(sig_mats[matrices[mut_type]], axis=1), columns=[mut_type]
            ),
            left_index=True,
            right_index=True,
            how="left",
    ...
    cohorts = [
        folder
        for folder in os.listdir(sig_dir)
        if not folder in ["eofiles", "Summary", "Pan"]
    ]

    for i, cohort in enumerate(cohorts):
        cohort_matrix = pd.read_csv(
            f"{sig_dir}/{cohort}/sigs_1_to_15_bestrun_inclFailed/CNV48/Samples.txt",
            sep="\t",
            index_col=0,
        )
        if i == 0:
            sig_mat = cohort_matrix.copy()
        else:
            sig_mat = pd.merge(
                sig_mat, cohort_matrix, left_index=True, right_index=True, how="inner"
            )
    sig_mats["CNV48"] = sig_mat.T
    sig_mats["CNV48"].set_index(
        sig_mats["CNV48"].index.map(lambda x: "_".join(x.split("_")[3:])), inplace=True
    )
    sig_mats["CNV48"] = (
        pd.merge(
            sig_mats["CNV48"],
    ...
    sig_totals = pd.merge(
        sig_totals,
        pd.DataFrame(np.sum(sig_mats["CNV48"], axis=1), columns=["CN"]),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    for i, cohort in enumerate(cohorts):
        cohort_matrix = pd.read_csv(
            f"{sig_dir}/{cohort}_S1_S15_R500_PoisNoisRes_T/SV32/Samples.txt",
            sep="\t",
            index_col=0,
        )
        if i == 0:
    ...
    sig_totals = pd.merge(
        sig_totals,
        pd.DataFrame(np.sum(sig_mats["SV32"], axis=1), columns=["SV"]),
        left_index=True,
        right_index=True,
        how="left",
    )
...
def plotMutationRates(
    sig_totals, groups=[], sig_types=["SBS", "DBS", "ID", "CN", "SV"]
):
...
if __name__ == "__main__":
    ...
    sample_df["sample_platekey"] = (
        ...
        + "_"
        + sample_df.tumour_sample_platekey.astype(str)
        + "_"
    ...
    if not os.path.exists(relabel_dir):
        os.mkdir(relabel_dir)
    for sig_set in combined_acts:
        relabel_set = pd.concat(
            (
                relabel_map["reference"][sig_set],
    ...
    tables, signatures = groupSignatureActivities(
        sample_df, combined_acts, cohorts="signature_extraction_group"
    )
    plotCohortActivities(tables, signatures, rename_sigs=relabel_map)
    publish_fig(f"signature_sizes_alltypes", publish=publish_dir)
    plotCohortActivities(
        tables, signatures, rename_sigs=relabel_map, orientation="vertical"
    )
    ...
    pearson_stat, pearson_pv, spearman_stat, spearman_pv = pearsonSpearmen(
        combined_act_df
    )
    ...
    sig_totals = pd.merge(
        sig_totals,
        sample_df[["signature_extraction_group", "sample_platekey"]],
        left_index=True,
        right_on="sample_platekey",
        how="inner",
    ).set_index("signature_extraction_group")
