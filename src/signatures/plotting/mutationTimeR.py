...
from signatures.utils import regexSearch, orderSignatures
from signatures.plotting.combinedSignatures import loadSignatures, map_colors, publish_fig
...
def loadMatrices(sample_list,
    ...
    full_matrix_df = {sig_type: {mut_time:pd.DataFrame() for mut_time in mut_times} for sig_type in sig_types}
        ...
        for mut_time in mut_times:
            ...
            if len(groups)>0:
                for group in groups:
                    ...
                    try:
                        matrix_df = pd.read_csv(f"{matrix_dir}/{mut_type}/{group}_{mut_time}.{sig_type}.all", sep="\t", index_col=0)
                    except pd.errors.EmptyDataError:
                        continue
                    if sig_type=='SBS4608':
                        matrix_df.set_index(matrix_df.index.map(lambda x: regexSearch("[A-Z]:[ACTG]([ACTG]\[[ACTG]>[ACTG]\][ACTG])[ACTG]", x, 1)), inplace=True)
                        matrix_df = matrix_df.groupby('MutationType').sum()

                    # Add matrix
                    full_matrix_df[sig_type][mut_time] = pd.concat((full_matrix_df[sig_type][mut_time], matrix_df.T))

                # Fill in missing samples
                full_matrix_df[sig_type][mut_time] = pd.merge(sample_list[[]], full_matrix_df[sig_type][mut_time],
                                                    left_index=True, right_index=True, how='left').fillna(0).astype(int)
            else:
                if sig_type=='SBS288':
                    matrix_df = pd.read_csv(f"{matrix_dir}/count_matrix_{mut_time}_{sig_type}.tsv", sep="\t", index_col=0)
                    matrix_df.set_index(matrix_df.index.map(lambda x: regexSearch("[A-Z]:([ACTG]\[[ACTG]>[ACTG]\][ACTG])", x, 1)), inplace=True)
                    matrix_df = matrix_df.groupby('MutationType').sum()
                    full_matrix_df[sig_type][mut_time] = matrix_df.T
                else:
                    full_matrix_df[sig_type][mut_time] = pd.read_csv(f"{matrix_dir}/count_matrix_{mut_time}_{sig_type}.tsv", sep="\t", index_col=0).T

        idx = full_matrix_df[sig_type]['clonal_[NA]'].index
        full_matrix_df[sig_type]['clonal'] = full_matrix_df[sig_type]['clonal_[NA]'].loc[idx]\
...
def getSigMutationRates(sample_list, combined_acts, combined_sigs, full_matrix_df,
                        mut_times=['clonal', 'subclonal', 'clonal_[early]', 'clonal_[late]'],
                        numerator='subclonal', denominator='clonal', sig_types = ['SBS288', 'DBS78', 'ID83']):
    ...
    for sig_type in sig_types:
        ...
        acts.set_index(acts.index.map(lambda x: "_".join(x.split("_")[1:3])), inplace=True)
        ...
        groups, group_idx = np.unique(sample_list.loc[platekeys].group, return_inverse=True)
        ...
        for timer_type in R_sample.keys():
            ...
            muts = np.array(full_matrix_df[sig_type][timer_type].loc[platekeys, channels])
            R_sample[timer_type] = pd.merge(R_sample[timer_type],
                                    pd.DataFrame((f @ muts[:,:,None])[:,:,0], columns=signatures, index=platekeys),
        ...
        for timer_type in R.keys():
            # Stacked mutations for clonal and subclonal
            muts = group_matrix@np.array(full_matrix_df[sig_type][timer_type].loc[platekeys, channels])
            R[timer_type] = pd.merge(R[timer_type],
                                    pd.DataFrame((f @ muts[:,:,None])[:,:,0], columns=signatures, index=groups),
                                    left_index=True, right_index=True, how='outer')
...
def getFractions(sample_list, combined_acts, R, R_sample,
                    numerator='subclonal', denominator='clonal',
                    sig_types = ['SBS288', 'DBS78', 'ID83']):

    groups = np.unique(sample_list.group)

    nonzero_counts = pd.merge(sample_list[['group']],
                            R_sample[numerator]+R_sample[denominator]>0,
                            left_index=True, right_index=True, how='inner').groupby('group').sum()

    rank_dfs = {sig_type:pd.DataFrame(index=combined_acts[sig_rename[sig_type]].keys()) for sig_type in sig_types}
    stat_dfs = {sig_type:pd.DataFrame(np.zeros((len(combined_acts[sig_rename[sig_type]].keys()), len(groups))),
                                    index=combined_acts[sig_rename[sig_type]].keys(),
                                    columns=groups) for sig_type in sig_types}
    pv_dfs = {sig_type:pd.DataFrame(np.zeros((len(combined_acts[sig_rename[sig_type]].keys()), len(groups))),
                                    index=combined_acts[sig_rename[sig_type]].keys(),
                                    columns=groups) for sig_type in sig_types}
    ...
    for sig_type in sig_types:
        ...
        for group in groups:
            group_platekeys = np.array(sample_list.reset_index().set_index('group').loc[group].platekey)
            frac = R_sample[numerator].loc[group_platekeys]/(R_sample[numerator].loc[group_platekeys]+R_sample[denominator].loc[group_platekeys])
            ...
            group_rank_df = pd.DataFrame({'sig':sig_labels_ordered,
                        group:np.arange(1,len(sig_labels)+1)/(len(sig_labels)+1)}).groupby('sig').mean()

            rank_dfs[sig_type] = pd.merge(rank_dfs[sig_type], group_rank_df,
            ...
            for sig in signatures:
                ...
                if np.sum(sig_labels==sig)>0:
                    mwu = scipy.stats.mannwhitneyu(nonnan_frac[sig_labels==sig], nonnan_frac[sig_labels!=sig])
...
def plotFractions(sample_list, nonzero_counts, combined_acts,
                    R, R_sample, rank_dfs, pv_dfs, stat_dfs,
                    numerator='subclonal', denominator='clonal', relabel_map=None,
                    label='Subclonal fraction'):

    # High count signatures
    high_count_signatures = nonzero_counts.keys()[np.sum(nonzero_counts>20)>0]
    high_count_groups = np.unique(sample_list.group)[np.unique(sample_list.group, return_counts=True)[1]>50]

    print([len(np.intersect1d(high_count_signatures, combined_acts[sig_rename[sig_type]].keys())) for sig_type in rank_dfs])

    fig, axes = plt.subplots(2,3,figsize=(len(high_count_signatures)/2,2*len(high_count_groups)/2),
                            gridspec_kw={'width_ratios':\
                    [len(np.intersect1d(high_count_signatures, combined_acts[sig_rename[sig_type]].keys())) for sig_type in rank_dfs]},
                            sharey=True)
    ...
    PVMIN=-np.log10(0.05)
    PVMAX=10
    colors2 = cm.bwr(np.linspace(0.1, 0.9, 128))#plt.cm.bone_r(np.linspace(0., 1, 128))
    colors2[int(np.floor((1-PVMIN/PVMAX)/2 * 128)):\
            int(np.ceil((1+PVMIN/PVMAX)/2 * 128))] = [0.8,]*4
    ...
    for j in range(2):
        ...
        for i,sig_type in enumerate(list(rank_dfs.keys())):
            ...
            signatures = np.intersect1d(high_count_signatures, combined_acts[sig_rename[sig_type]].keys())
            ...
            rank_array = np.array(rank_dfs[sig_type].loc[signatures,high_count_groups].copy())
            sizes = np.array(nonzero_counts.T.loc[signatures,high_count_groups])

            pv_array = np.array(-np.log10(pv_dfs[sig_type].loc[signatures,high_count_groups].copy())) * \
            ...
            im = plt.scatter(xx.flatten()+0.5, yy.flatten()+0.5,
                            s=size_factor(sizes).T.flatten(),
                            c=np.array(R_num/(R_num+R_den)).T.flatten() if j==0 else pv_array.T.flatten(),
                            vmin=0. if j==0 else -PVMAX,
                            vmax=1. if j==0 else  PVMAX,
            ...
            cb_ax = fig.add_axes([0.96, 0.55, 0.015, 0.3]) if j==0 else fig.add_axes([0.96, 0.14, 0.015, 0.3])

            #cbar = plt.colorbar()
            cbar = fig.colorbar(im, cax=cb_ax)
            # cbar.set_label(r"$\mathrm{sgn}(\beta) \,\times\,-\log_{10}(\mathrm{pvalue})$", fontsize=36)
            cbar.set_label(label if j==0 else r"$-\log_{10}(P\mathrm{value})$",
                        fontsize=40)
            ...
            if (i==0)&(j==0):
                ...
                legend = ax.legend([l1,l2,l3,l4],[f'$1$',f'$10$',f'$100$',f'$1000$'],
                                    loc='lower left', bbox_to_anchor=(0,1),
                                    fontsize=fs, ncol=4, frameon=False, title="Number of samples")
...
if __name__=='__main__':
    ...
    stat_dfs={}
    pv_dfs={}
    nonzero_counts['subclonal'], rank_dfs['subclonal'], stat_dfs['subclonal'], pv_dfs['subclonal'] = getFractions(sample_list, combined_acts, R, R_sample,
                                                            numerator='subclonal', denominator='clonal')
    # Get rank statistics of signatures in groups
    nonzero_counts['late'], rank_dfs['late'], stat_dfs['late'], pv_dfs['late'] = getFractions(sample_list, combined_acts, R, R_sample,
                                                            numerator='clonal_[late]', denominator='clonal_[early]')

    # Save a single table
    timer_df = pd.DataFrame()
    for sig_type in ['SBS288', 'DBS78', 'ID83']:
        pv_df = pv_dfs['subclonal'][sig_type].stack().reset_index().rename({"level_0":"signature",
                                                                "level_1":"group",
                                                                0:"pv_subclonal"}, axis=1)
        rank_df = rank_dfs['subclonal'][sig_type].stack().reset_index().rename({"level_0":"signature",
                                                                "level_1":"group",
                                                                0:"rank_subclonal"}, axis=1)
        pv_df = pd.merge(pv_df,
                        pv_dfs['late'][sig_type].stack().reset_index().rename({"level_0":"signature",
                                                                "level_1":"group",
                                                                0:"pv_late"}, axis=1),
                        on=['signature', 'group'], how='inner')
        rank_df = pd.merge(rank_df,
                        rank_dfs['late'][sig_type].stack().reset_index().rename({"level_0":"signature",
                                                                "level_1":"group",
                                                                0:"rank_late"}, axis=1),
                        on=['signature', 'group'], how='inner')
    ...
    plotFractions(sample_list, nonzero_counts['subclonal'], combined_acts, R, R_sample, rank_dfs['subclonal'], pv_dfs['subclonal'], stat_dfs['subclonal'],
                                numerator='subclonal', denominator='clonal',
                                relabel_map=relabel_map, label='Subclonal fraction')
    publish_fig('subclonal_fraction_rank_mutationtimer', publish=FIGURE_DIR)

    # Plot clonal fraction and rank statistics
    plotFractions(sample_list, nonzero_counts['late'], combined_acts, R, R_sample, rank_dfs['late'], pv_dfs['late'], stat_dfs['late'],
                                numerator='clonal_[late]', denominator='clonal_[early]',
                                relabel_map=relabel_map, label='Late clonal fraction')
