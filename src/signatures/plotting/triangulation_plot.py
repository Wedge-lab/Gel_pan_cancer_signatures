...
if __name__=='__main__':
    ...
    # 4) Change x-labels
    relabel_x = {'Chromatin Structure and Modification':'Chromatin struct. & mod.',
                'DNA polymerases ':'DNA polymerases',
                'Homologous recombination':'HR',
                'grade':'Grade', 'stage_best':'Stage',
                'er_status':'ER', 'pr_status':'PR', 'triple_negative':'Triple negative',
                'survival':'Survival', 'subclonal':'Subclonal', 'late':'Late clonal',
                'is_WGD':'WGD', 'n_chromothripsis':'Chromothripsis',
                'n_chromoplexy':'Chromoplexy', 'n_tandem_duplication':'Tandem duplications', 'n_kataegis':'Kataegis'}
    ...
    titles = {'gene':'Gene inactivation', 'genotype':'Genomic alterations', 'treatment':'Treatment',
            'clinical':'Status', 'survival':'Survival', 'timer':'Timing'}
    result_colors = {'gene':0, 'genotype':0, 'treatment':0, 'clinical':0, 'survival':0, 'timer':0}
    cmaps = {0:['bwr', -10, 10], 1:['PRGn', -5, 5], 2:['RdBu',-1,1]}
    triang_dict = {}
    for j,result_type in enumerate(result_types+['index','colour_id']):
        triang_dict[result_type] = pd.read_csv(f"/re_gecip/shared_allGeCIPs/pancancer_signatures/results/figs/triangulation/{result_type}.tsv",
                                                sep='\t', index_col=0)
        not_null_count = (~pd.isnull(triang_dict[result_type])).sum()
        triang_dict[result_type] = triang_dict[result_type][not_null_count[not_null_count!=0].index]
    ...
    for j,result_type in enumerate(result_types):
        ...
        plt.scatter(xpos + xx.flatten()+0.5, yy.flatten()+0.5,
                    c=np.array(triang_dict[result_type][sig_subset]).flatten(), cmap=cmap_variables[0],
                    vmin=cmap_variables[1], vmax=cmap_variables[2])
        ylim = [0,triang_dict[result_type].shape[0]]
        print(result_type, ylim)

        xtick_labels += list(triang_dict[result_type][sig_subset].rename(relabel_x, axis=1).keys())
        plt.text(xpos+triang_dict[result_type][sig_subset].shape[1]/2-0.5, len(y)+0.5, titles[result_type],
                ha='left', va='bottom', rotation=45, fontsize=12)
    ...
    cbar = fig.colorbar(cm.ScalarMappable(norm=Normalize(vmin=cmaps[0][1], vmax=cmaps[0][2]), cmap=cmaps[0][0]),
                        cax=cax, orientation='vertical')
    cbar.ax.tick_params(labelsize=fs)
    cbar.set_label(fr"Z-score aggregated across cohorts", fontsize=fs*1.5)
    ...
    ax.set_yticklabels(triang_dict['index'][sig_subset].index, fontsize=10, minor=False, rotation=0);
    ...
    for label, gene_group in group_labels.items():
        intersection = np.intersect1d(triang_dict['index'][sig_subset].index, gene_group, return_indices=True)
        plt.text(-4, float(np.mean(intersection[1]))+0.5, label, ha='center',va='center', fontsize=fs*1.2,
                color=default_colours[triang_dict['colour_id'].colour_id[sig_subset].loc[gene_group].iloc[0]+1],
                rotation=45)
